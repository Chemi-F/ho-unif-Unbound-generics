...
-- generally, use lfresh then we can get fresh variable name
--
-- avoid (F, G) H --> H
-- avoid (F, H) H --> H1
...
occ _F th t = occ' t
    where
        ...
        occ' (Abs b)   = let ids = toListOf fv (Abs b) :: [ID]
                         in elem _F ids
        occ' _         = False
...
flexflex2 _F xm _G yn th = do
    ...
    if xm `subset` yn then do
        ...
        let fvs = (toListOf fvAny (F _F)) ++ (toListOf fvAny (F _G))
        let zk = xm `cap` yn
        _H  <- newFv fvs
        lxH <- hnf xm _H zk
        lyH <- hnf yn _H zk
...
unif s t =
    ...
    in unif' [] (s', t')
    where
        v2f :: Term -> Term -- V ID (free variable) -> F ID
        v2f t = let fvs = toListOf fv t :: [ID]
                in foldl (\t f -> subst f (F f) t) t fvs
        unif' th (s, t) = do
            prth <- prTheta th
            prs  <- prTerm s
            prt  <- prTerm t
...
testunbind (Abs b) = unbind b

testlunbind (Abs b) = lunbind b $ \(x, t) -> return (x, t)
